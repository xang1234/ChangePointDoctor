...
mod tests {
    ...
    fn naive_l2_multivariate_c(
        ...
        for dim in 0..d {
            ...
            total += naive_l2_univariate(&segment, 0, segment.len());
        }
        assert!(n > 0);
        total
    }
    ...
    fn multivariate_matches_univariate_sum_and_d1_parity() {
        ...
        let d1_naive = naive_l2_univariate(&d1_values, 1, 5);
        ...
        let naive = naive_l2_multivariate_c(&values, n, d, 1, 5);
        assert_close(fast, naive, 1e-9);
    }
