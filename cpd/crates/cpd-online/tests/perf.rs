...
const LOG_PROB_THRESHOLD: f64 = -35.0;
...
fn perf_signal(step: usize) -> f64 {
    let regime = (step / 32) % 2;
    if regime == 0 { 0.0 } else { 6.0 }
}
...
fn bocpd_gaussian_perf_contract() {
    ...
    println!(
        "BOCPD perf: steps={} elapsed_s={:.6} updates_per_sec={:.2} p99_update_us={} max_run_length={} enforce={}",
        MEASURE_STEPS,
        elapsed.as_secs_f64(),
        updates_per_sec,
        p99_update_us,
    ...
    emit_metrics_if_requested(updates_per_sec, p99_update_us, enforce)
